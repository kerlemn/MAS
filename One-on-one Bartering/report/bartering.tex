...
Due to computational complexity, experiments were run with $N=2$ goods per agent and $K=15$ types. For $N=3$, the algorithm did not terminate in reasonable time.

\subsubsection*{Verification}

For a fixed preference profile of agent~1 and a selected type of agent~2, all outcomes in $O$ were analyzed\textsuperscript{Fig \ref{fig:result}}. Each outcome was reported in the form "$\text{outcome:status $\rightarrow$ reason}$", where the status indicates whether the outcome belongs to the computed set $CB$, is excluded (\textbf{X}), is the outcome selected by the mechanism (\textbf{*}), or is an alternative yielding identical utilities for both agents (\textbf{$\sim$}).
